...
export default function SocialLink({ social }: { social: Social }) {
  return (
    <Link
      ...
      href={social.href}
      target="_blank"
      className={`group relative min-h-[150px] cursor-none rounded-3xl border transition-all duration-300 hover:border-transparent ${social.class} flex items-center justify-center overflow-hidden`}
    >
      <div className="absolute inset-0 opacity-0 transition-opacity duration-300 group-hover:opacity-100"></div>
      <social.icon className="text-primary/80 relative z-10 size-10 transition-all duration-300 group-hover:scale-110 group-hover:text-white" />
